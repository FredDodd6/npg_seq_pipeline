...
sub	get_table_info_for_id_run {
  ...
  die "need an id_run" unless $id_run;

  my$rs=$npg_tracking_schema->resultset(q(Run));
  my$fn=$rs->find($id_run)->folder_name;

  my $rsIseq=$mlwh_schema->resultset("IseqProductMetric")->search({"id_run" => $id_run},
        	{join=>{"iseq_flowcell" => "sample"}});

  my @table_info = ($fn,$rsIseq);
  return (@table_info);
}

sub get_majora_data {
  my $fn = $_[0];
  my @curl_command = ('curl', '-s', '-L');
  ...
  close $fh;
  $curldata or die"nothing back from curl";
  return($curldata);
}
  ...
  while (my $row=$rsIseq->next) {
    ...
    if ($libdata) {
      ...
      if ($sample_data) {
        ...
        warn "setting $sample_meta for ". $fc->sample->supplier_name;
      }
    }
